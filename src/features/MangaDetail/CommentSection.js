...
import { deleteComment, getComments, getReplies, likeComment, postComment, unlikeComment } from '~/services/commentService';
    ...
    <div className={cx('commentWrapper', { [`level-${level}`]: level > 1 })}>
          ...
          <div className={cx('commentActions')}>
            ...
            {(comment.totalReplies > 0 || replies.length > 0) && (
              <button className={cx('actionBtn', 'replyToggle')} onClick={toggleReplies}>
                ...
                <span>
                  {showReplies ? 'Ẩn' : 'Xem'} {replies.length > 0 ? replies.length : comment.totalReplies} trả lời
                </span>
              </button>
            )}
          </div>
      ...
      {showReplyInput && (
        <div className={cx('replyInput')}>
          <input type="text" className={cx('inputField')} value={replyText} onChange={(e) => setReplyText(e.target.value)} placeholder="Viết trả lời..." />
